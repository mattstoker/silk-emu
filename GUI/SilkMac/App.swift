...
class System: ObservableObject {
    ...
    init(cpu: CPU6502 = CPU6502(load: { _ in 0 }, store: { _, _ in }), memory: [UInt8] = Array((0x0000...0xFFFF).map { _ in UInt8.random(in: 0x00...0xFF) })) {
        self.memory = memory
        self.cpu = CPU6502(
            pc: cpu.pc,
            ac: cpu.ac,
    ...
    func screenshot() -> NSImage {
        let ppm = Self.screenshot(cpu: cpu, start: 0x2000, end: 0x4000, width: 0x7F)
                ...
                .fileImporter(isPresented: $programImporterShowing, allowedContentTypes: [.data]) { result in
                    ...
                    case .success(let url):
                        ...
                        for index in program.indices {
                            system.memory[programOffset + index] = program[index]
                        }
                    }
                }
                if stepTimer == nil {
